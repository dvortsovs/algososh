...
import {IQueueState} from "../../types/queue-state";
...
export const QueuePage: React.FC = () => {
    ...
    const addItem = (e: FormEvent, item: typeof value) => {
        e.preventDefault()
        if (!value || isLoad || (queueState[queueState.length - 1].item || queueState[queueState.length - 1].item === '')) return
        setIsLoad(true)
        ...
        setTimeout(() => {
            queueStateCopy[queue.current.getTailIndex() - 1].state = ElementStates.Default
            setQueueState([...queueStateCopy])
        }, DELAY_IN_MS)
    }
    ...
    const deleteItem = () => {
        ...
        if (queue.current.getLength() === 1) {
            setTimeout(() => {
                ...
                queueStateCopy[queue.current.getHeadIndex()].state = ElementStates.Default
                setQueueState([...queueStateCopy])
            ...
            <section className={styles.main}>
                <form className={styles.form} onSubmit={(e) => addItem(e, value)}>
                    <Input isLimitText={true}
                           ...
                           value={value}/>
                    <Button disabled={value.length <= 0 || isLoad}
                            onClick={(e) => {
                                addItem(e, value)
                            }}
                            isLoader={false}
                            text={'Добавить'}
                    />
                    <Button disabled={queue.current.getLength() <= 0 || isLoad}
                            onClick={deleteItem}
                            isLoader={false}
                            text={'Удалить'}/>
                    <div className={styles.clear}>
                        <Button
                            disabled={(queue.current.getLength() <= 0
                                &&
                                !queueState.map((item) => (item.item)).includes('')) || isLoad}
                            text={'Очистить'}
                            onClick={clearQueue}/>
                    </div>
                </form>
                        ...
                        return (
                            <li key={index} className={styles.item}>
                                <Circle state={item.state}
                                        ...
                                        head={item.item === ''
                                            ? 'head'
                                            : item.item && index === queue.current.getHeadIndex() ? 'head' : ''}
                                        tail={item.item && index === queue.current.getTailIndex() - 1 ? 'tail' : ''}
                                />
                            </li>
                        )
                    })}
